# VibeLoom Codex V3 Package

This subtree contains the final merged Codex package for VibeLoom. It uses `/codex-plus` as the structural and semantic base, then selectively incorporates the strongest teaching, onboarding, and template ideas from `/claude-plus` without weakening the reconciled methodology.
...
| `spec.md` | Technical design for the Codex V3 package and future runtime protocol |
...
- `/codex-v3` is the new merged target package.
- `/codex-plus` remains the semantic base and comparison point.
- `/claude-plus` remains a reference source for selected teaching and template ideas only.
