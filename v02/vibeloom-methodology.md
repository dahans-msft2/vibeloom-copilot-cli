...
| `bdd` | Non-executable behavioral Gherkin scenarios generated from contract. Could be used by users and agents to generate executable behavioral tests. |
...
`eval` and `generate` are the two primitives. `review` is an interactive shell on top of `eval`; `reconcile` is an interactive shell on top of `generate`.
...
- **Precondition:** Repo has no existing VibeLoom governance.
- **Postcondition:** Mode set. Draft `intent` and `defaults` generated from the seed or interview.
...
- **Purpose:** Generate one or more affected tiers from approved upstream truth using the forward-back pass model.
...
- **Flags:** None.
- **Precondition:** Upstream tier(s) approved (or, for `intent-specs`, user seed provided).
- **Postcondition:** Target tier artifacts in `draft` state; `eval` runs automatically on generated artifacts.
...
- **Purpose:** Detect and resolve downstream drift from approved upstream changes. `reconcile` is an interactive shell on top of `generate` — each cycle detects drift, gets user-selected fix direction, then invokes `generate` on affected artifacts.
...
- **Parameter:** Approval unit (`intent-specs` | `product-specs` | `system-specs`).
- **Flags:** None.
- **Precondition:** Structural `eval` passes (all blocking checks clear).
- **Postcondition:** Approval unit status set to `approved`; approval provenance recorded.
...
- **Postcondition:** Read-only report; no state changes.
...
#### vibe mode
In `vibe`, the flow is: `init` → `approve intent-specs` → `generate code` (which auto-generates compact `system-specs` and `context` implicitly).
...
If/when the application outgrows the vibe, the user can upgrade to full ceremony by changing configuration to one of the "full" modes.
