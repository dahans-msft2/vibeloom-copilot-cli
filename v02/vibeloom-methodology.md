...
- Repo-scoped: `intent`, `defaults`, `prd`, `usm`, `dm`, `system`, `containers`, `bdd`, `pdr`, `adr`. Container-scoped: `container`, container-level `config`. Component-scoped: `component`, component-level `config`.
- Affected artifacts, tiers, and scopes are computed from the change set by ownership lookup.
...
In full modes, the change set for a change is computed by walking derivation edges forward from every changed entity in the graph and collecting all reachable entities.

Affected artifacts, tiers, and scopes are then computed from the change set by ownership lookup.
...
VibeLoom defines eight methodology-level operations. Implementations may expose them through different commands or interfaces, but the logical operations stay the same. Exact CLI surface, file formats, and argument syntax belong to implementation, not to methodology.

`eval` and `generate` are the two primitives. `review` is an interactive shell on top of `eval`; `reconcile` is an interactive shell on top of `generate`. An approval unit is one contract tier.
...
- **Parameter:** Optional target (`intent-specs` | `product-specs` | `system-specs` | `context` | `code`). When omitted, generates the lowest tier that is either not yet generated or stale.
- **Flags:** None.
...
- When a contract tier is generated, affected context artifacts are regenerated automatically.
...
- **Purpose:** Run structural and semantic validation on the current approval unit. This is the validation primitive.
- **Parameter:** Optional approval unit (`intent-specs` | `product-specs` | `system-specs`). When omitted, evaluates the lowest tier currently in `draft`.
- **Flags:** None.
- **Precondition:** Approval unit exists (in `draft` or `approved`).
- **Postcondition:** Structural findings (blocking) and semantic findings (non-blocking) reported. No artifacts modified.
- Runs automatically as part of `generate` and `approve`; explicit invocation is for targeted checks outside the normal flow.
- In `vibe`: valid values are `intent-specs` | `system-specs`.

### `review`

- **Purpose:** Interactive validation of the current candidate approval unit. `review` is an interactive shell on top of `eval` — each review cycle starts by running `eval`, then surfaces findings, proposes fixes, and applies bounded edits within the approval unit.
- **Parameter:** Optional approval unit (`intent-specs` | `product-specs` | `system-specs`). When omitted, reviews the lowest tier currently in `draft`.
- **Flags:** None.
- **Precondition:** Approval unit exists in `draft`.
- **Postcondition:** Findings surfaced; bounded fixes applied within the approval unit.
- Does not propagate changes downward; that belongs to `reconcile`.
- In `vibe`: valid values are `intent-specs` only. Downstream compact `system-specs` and code are never edited directly by review.
...
- In `vibe`: valid targets are `system-specs` | `context` | `code`.

### `approve`

- **Purpose:** Advance a reviewed contract approval unit from `draft` to `approved`.
- **Parameter:** Optional approval unit (`intent-specs` | `product-specs` | `system-specs`). When omitted, approves the lowest tier currently in `draft` whose structural eval passes.
- **Flags:** None.
- **Precondition:** Structural `eval` passes (all blocking checks clear).
...
- In `vibe`: valid values are `intent-specs` | `system-specs` (though `system-specs` typically auto-advances).

### `status`

- **Purpose:** Show current lifecycle state, graph health, staleness, and coverage gaps.
...
- **Postcondition:** Read-only report; no state changes. The report includes:
  - Per-tier lifecycle state (`draft` | `approved` | not yet generated)
  - Stale artifacts (downstream of changed upstream)
  - Coverage gaps (upstream entities with no downstream derivation)
  - Graph health (dangling references, missing required fields)
  - Current mode
...
| --- | --- | --- |
| `review` — interactive shell on `eval` | `eval` — structural and semantic validation | current approval unit |
...
In `vibe`, review and eval use heuristic evidence rather than graph-backed validation: filesystem layout, exported interfaces, route or command names, tests, key strings, and owned-path comparisons. These checks are not code-item validation; unresolved findings may lead to further intent revision, reconciliation, or upgrade.

### Eval

Eval runs the forward-back pass (see above) across the current approval unit but reports findings instead of regenerating. Findings are classified as structural (blocking) or semantic (non-blocking).
...
VibeLoom workflows define how the methodology operations compose into end-to-end flows.
...
#### vibe mode
In `vibe`, the flow is: `init` → `approve intent-specs` → `generate code`. The `generate` command auto-advances compact `system-specs` and `context` before producing code (see Approval And Auto-Advance).

### Brownfield Import

Brownfield import is the bootstrap path for unmanaged or heavily drifted repos. It reconstructs candidate contract bottom-up from existing code and marks uncertainty explicitly for user review. Once all tiers are approved and reconciliation is resolved, normal top-down governance takes over (see Developing A Governed Project).
...
3. `generate` produces updated artifacts for affected tiers (forward-back pass), starting from the highest affected tier downward.
4. `eval` runs automatically on each generated tier. User iterates via `review` or out-of-band edits as needed.
5. User runs `approve` on each affected contract tier (or auto-advance handles delegated tiers per mode).
6. Affected context artifacts are regenerated automatically from the approved contract.
