---
name: vibeloom-dev
description: Develops vibeloom itself. Adversarial consistency checks and generation across intent / canon / skill / site for any vibeloom version. Use to bring up a new version, eval/review canon for drift, generate downstream artifacts from upstream specs, or run cross-agent reviews (Claude + Codex).
argument-hint: "[init|eval|review|generate|reconcile|feedback] [target|peer-agent] [target] [--version vNN] [--from vNN] [--from-scratch]"
---
...
- critique the other agent's eval (`feedback <peer> <target>`)
...
- **Default:** operates on the **latest mutable `vNN`** — the highest-numbered version directory that is NOT current production (where production = target of root `site/public/` symlink).
- **Override:** `--version vNN` (e.g., `--version v04`).
...
If you've just installed this skill and want to try it on the current vibeloom version:

```
vibeloom-dev eval canon
```

This runs an adversarial consistency check on the latest mutable vibeloom version's canon (intent + manifesto + methodology + implementation). Findings land in `reports/eval-canon-<your-agent>.md`. Then `vibeloom-dev review canon` walks them.

To bring up a new version (e.g., v05 from v04):

```
vibeloom-dev init
```

This copies v04 → v05 with version refs updated, then interviews you to refactor `intent.md`.
